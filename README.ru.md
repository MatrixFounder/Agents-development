...
# Мультиагентная система разработки v3.2.5
...
### 3. Исполняемые Навыки (Инструменты)
Новое в v3.2: Система поддерживает **Нативные Инструменты**, выполняемые Оркестратором (на основе схем).
- **Определение**: `.agent/tools/schemas.py`.
- **Возможности**: Запуск тестов, Git-операции, работа с файлами.

**[>> Руководство по Оркестратору и Инструментам <<](docs/ORCHESTRATOR.md)** (Конфигурация, Новые инструменты и Устранение неполадок)
...
2.  **Навыки**: Создайте символическую ссылку для включения нативных навыков:
    ```bash
    ln -s .agent/skills .cursor/skills
    ```
    *   *Примечание:* Это позволит Cursor индексировать навыки, сохраняя `.agent/skills` как единый источник правды.
...
```bash
# Обязательно для выполнения инструментов
pip install pytest  # Для инструмента run_tests

# Опционально: для AI-оркестрации (если используете собственные скрипты)
...
├── .agent/skills/                 # [Common] Библиотека навыков
├── .agent/workflows/              # [Common] Сценарии
