...
# Мультиагентная система разработки v3.0.2
...
| **Ревьюер ТЗ** | `03_tz_reviewer_prompt.md` | Контроль качества Технических Заданий. |
...
3. **Artifacts:**
   ...
   - **Архивируйте ТЗ:** Скопируйте финальное ТЗ в архив: `cp docs/TZ.md docs/tasks/task-ID-name.md`.
...
|----------|------|--------|--------------|
| **Техническое Задание (ТЗ)** | `docs/TZ.md` | **Единый Источник Истины (Текущая Задача)** | **СТРОГО для текущей активной задачи**. Перезаписывать при обновлениях. В архив перед новой задачей. |
...
1. **Одна Задача = Одно ТЗ**: `docs/TZ.md` всегда отражает *только* то, что модифицируется прямо сейчас.
2. **Стратегия Архивации**:
   - **Перед** началом фундаментально новой задачи: Архивируй `docs/TZ.md` -> `docs/tasks/task-00N-name.md`.
   - **Во время** задачи: Только перезаписывай `docs/TZ.md`. Никогда не дописывай в конец.
3. **Правило уборки**:
