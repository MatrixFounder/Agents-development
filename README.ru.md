...
# Мультиагентная система разработки v3.5.5
...
2.  **Навыки**: Создайте символическую ссылку:
    ```bash
    ln -s .agent/skills .cursor/skills
    ```

#### 🟣 Вариант Б: Antigravity (Нативно)
1.  **Конфигурация**: Скопируйте `.gemini/GEMINI.md` в корень проекта (системный промпт).
2.  **Навыки**: Antigravity загружает навыки из `.agent/skills/` автоматически.
3.  **Автономный режим**: Добавьте в **Allow List Terminal Commands**:
    ```text
...
├── .agent/
│   ├── skills/                  # [Common] Библиотека навыков
