...
# Мультиагентная система разработки v3.9.12
...
- [Практическое использование (Claude Code & Gemini)](#practical-usage)
- [Управление артефактами](#artifact-management)
- [Что делать с файлами .AGENTS.md?](#agents-files)
- [Подготовка к итерациям](#future-iterations)
- [Reverse Engineering](#reverse-engineering)
- [Шаблоны промптов](#starter-prompts)
- [Миграция со старых версий](#migration-guide)
...
1.  **Конфигурация**: Создайте алиас `CLAUDE.md` для системного промпта:
    ```bash
    ln -s AGENTS.md CLAUDE.md
    ```
2.  **Навыки**: Создайте симлинк `.claude/skills` (опционально, для совместимости):
    ```bash
    mkdir -p .claude
    ln -s ../.agent/skills .claude/skills
    ```
3.  **Использование**: Запустите `claude` в корне проекта. Системный промпт загрузится автоматически.
...
| **Claude Code** | `CLAUDE.md` (симлинк) | Автоматически (при запуске) |
