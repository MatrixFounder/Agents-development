...
2. Внимательно изучи постановку задачи
3. Изучи описание существующего проекта (если есть)
4. Выяви все неясные моменты и сформулируй вопросы
5. Создай структурированное ТЗ
6. **СТРОГОЕ ПРАВИЛО:** ВСЕГДА ПЕРЕЗАПИСЫВАЙ `docs/TZ.md` полностью.
   - НИКОГДА не добавляй (append) к существующему файлу.
   - Если это новая задача, считай, что Оркестратор уже архивировал предыдущее ТЗ. Твоя работа — создать НОВОЕ с нуля.
...
5. **ОБЯЗАТЕЛЬНО:** ВСЕГДА обновляй файл `docs/TZ.md` с внесенными исправлениями.
   - ПЕРЕЗАПИСЫВАЙ файл полностью обновленным контентом.
   - Не просто выводи текст в чат.
   - Не добавляй (do not append).
...
### 0. Мета-информация
- **Task ID:** Определи на основе существующих задач в docs/tasks/ (например, 002, если 001 существует) или 001 для нового проекта.
- **Slug:** Краткое название в kebab-case (например, `user-auth`).
...
- **ОБЯЗАТЕЛЬНО:** Ты должен ВСЕГДА сохранять/обновлять содержимое в `docs/TZ.md` локально.
