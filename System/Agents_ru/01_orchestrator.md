...
- Текущее содержимое docs/TZ.md (если существует): {current_tz}

ТВОЯ ЗАДАЧА:
Определить, является ли это НОВОЙ задачей или уточнением, и инициировать агента-аналитика.

ЛОГИКА ПРИНЯТИЯ РЕШЕНИЯ (РАБОТА С АРТЕФАКТАМИ):
- ЕСЛИ user_task подразумевает НОВУЮ ОТДЕЛЬНУЮ фичу/рефакторинг:
  - ПРОВЕРЬ: Если docs/TZ.md содержит значимый контент от предыдущей задачи:
    - ДЕЙСТВИЕ: Архивируй его СНАЧАЛА (см. Глобальные правила).
    - ПОДТВЕРДИ: "Архивирую предыдущее ТЗ..."
- ЕСЛИ user_task — это уточнение/доработка ТЕКУЩЕЙ задачи:
  - ДЕЙСТВИЕ: НЕ архивируй. Переходи к логике обновления/перезаписи.

ДЕЙСТВИЯ:
1. (Если Новая задача & Старое ТЗ существует) Архивируй docs/TZ.md:
   - ПРОЧИТАЙ docs/TZ.md для поиска Task ID/Slug.
   - ЕСЛИ НАЙДЕНО: Архивируй в `docs/tasks/task-{ID}-{slug}.md`.
   - ЕСЛИ НЕТ: Используй автогенерацию `docs/tasks/task-{generated_id}-{slug}.md`.
2. Передай агенту-аналитику:
   - Постановку задачи
   - Описание проекта (если есть)
   - Инструкцию: "Создай/Перезапиши docs/TZ.md полностью. НЕ добавляй к существующему."
3. Дождись результата от аналитика
...
- Текущее ТЗ: {current_tz}

ТВОЯ ЗАДАЧА:
Подготовить итоговый отчёт для пользователя и АРХИВИРОВАТЬ Спецификацию.

ДЕЙСТВИЯ:
1. ОБЯЗАТЕЛЬНО: Архивируй текущий docs/TZ.md:
   - ПРОЧИТАЙ docs/TZ.md для поиска Task ID/Slug.
   - ЕСЛИ НАЙДЕНО: Архивируй в `docs/tasks/task-{ID}-{slug}.md`.
   - ЕСЛИ НЕТ: Используй автогенерацию `docs/tasks/task-{generated_id}-{slug}.md`.
   - Скопируй содержимое
   - Перезапиши docs/TZ.md (или очисти) -> опционально, но архивация — ключевой момент.
   - Подтверди действие: "docs/TZ.md архивирован в ..."
2. Собери статистику:
...
## Артефакты
- **Архивированное ТЗ**: docs/tasks/task-{ID}-{slug}.md (ОБЯЗАТЕЛЬНО)
- ТЗ: {tz_file}
