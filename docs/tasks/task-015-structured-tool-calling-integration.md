### 0. Meta Information
- **Task ID:** 015
- **Slug:** structured-tool-calling-integration
- **Status:** Completed
...
5. **Workflows & Automation (New)**
   - **Legacy Cleanup:** Заменить старые термины `tz` на `task` во всех workflow файлах (`.agent/workflows/*.md`).
   - **Full Automation:** Создать workflow `/run-full-task`, который:
     1. Считывает список задач из `docs/PLAN.md` или `docs/tasks/`.
     2. Последовательно запускает цикл разработки (Developer -> Reviewer) для каждой задачи.
     3. Останавливается при ошибке.
